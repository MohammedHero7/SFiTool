...
class AtlasViewer(QWidget):
    """Widget that displays atlas pixmap with grid and supports clicks."""
    ...
    def __init__(self, parent=None):
        ...
        self.rows = 0
        self.padding = 0
        self.selected = None  # index (row, col)
        self.overlay_map = {}  # glyph_index -> unicode int
    ...
    def _update_scale(self):
        ...
        # Calculate scale to fit image in widget maintaining aspect ratio
        scale_w = widget_w / max(1, img_w)
        scale_h = widget_h / max(1, img_h)
        self.scale = min(scale_w, scale_h, 1.0)  # Don't upscale beyond actual size

    def resizeEvent(self, ev):
        """Recalculate scale when widget is resized."""
    ...
    def paintEvent(self, ev):
        ...
        # Draw scaled pixmap
        scaled_pixmap = self.atlas_pixmap.scaledToWidth(
        ...
        if self.atlas_image:
            ...
            for r in range(self.rows + 1):
                y = int(r * (self.cell_h + self.padding) * self.scale)
                painter.drawLine(0, y, int(self.atlas_image.size[0] * self.scale), y)
            
            # Draw vertical lines
            for c in range(self.cols + 1):
                x = int(c * (self.cell_w + self.padding) * self.scale)
                painter.drawLine(x, 0, x, int(self.atlas_image.size[1] * self.scale))
        ...
        if self.selected:
            ...
            x = int(c * (self.cell_w + self.padding) * self.scale)
            y = int(r * (self.cell_h + self.padding) * self.scale)
            w = int(self.cell_w * self.scale)
            h = int(self.cell_h * self.scale)
            painter.fillRect(x, y, w, h, QColor(255, 200, 0, 60))
            ...
            for c in range(self.cols):
                ...
                if u is not None:
                    ...
                    if ch:
                        x = int(c * (self.cell_w + self.padding) * self.scale)
                        y = int(r * (self.cell_h + self.padding) * self.scale)
                        painter.drawText(x + int(4 * self.scale), y + int(14 * self.scale), ch)

    def mousePressEvent(self, ev):
        if not self.atlas_pixmap or self.scale <= 0:
            return
        # Convert widget coordinates to image coordinates accounting for scale
        x = ev.position().x() / self.scale
        y = ev.position().y() / self.scale
        c = int(x // (self.cell_w + self.padding))
        r = int(y // (self.cell_h + self.padding))
    ...
    def dropEvent(self, ev):
        # Accept dropped file: single PNG as atlas
        for url in ev.mimeData().urls():
            path = url.toLocalFile()
            if path.lower().endswith('.png'):
                try:
                    img = Image.open(path).convert('RGBA')
                    # if no cell size defined, try auto-detect
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Font Image Editor - My tool")
